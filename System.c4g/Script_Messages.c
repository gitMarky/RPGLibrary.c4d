...
{
	Message(ColorizeString(Format( "<%s>|%s", GetName(pObj),szString),GetPlrColorDw(GetOwner(pObj))),pObj);
}
...
global func MsgBox(object pPlayer, string sMessage, object pSpeaker, string szPortrait, bool fAsMessage, bool fPermanent, array aOptions)
{
	  ...
	  var idMenuDeco = MD69;
	  var szMenuCaption = "";
	  if( !( idMenuDeco = (pSpeaker->~GetDlgMsgDeco()) )) idMenuDeco = MD69;
	  ...
	  if (fAsMessage)
	  {
			...
			CustomMessage(Format("<c %x>%s:</c> %s", dwClr, GetName(pSpeaker), sMessage), 0, GetController(pPlayer), 0,35, 0xffffff, idMenuDeco, szPortrait,MSG_HCenter|MSG_Top );
	  ...
	  else
	  {
			...
			StopClonkEx(pPlayer);
			if (!CreateMenu(_CAM, pMsgTarget, this, 0, szMenuCaption, 0, C4MN_Style_Dialog, fPermanent, _CAM)) return false;
			if(GetType(aOptions) == C4V_Array)
			{
				...
				dwColor = aOptions[1];
				fInstant = aOptions[2];
				if( aOptions[3] ) szPortrait = aOptions[3];

				var szMsgBox = sMessage;
				if( dwColor ) szMsgBox = ColorizeString( szMsgBox, dwColor );
				if( fName ) szMsgBox = Format("<c %x>%s:</c> %s", dwClr, GetName(pSpeaker), szMsgBox);
				...
				{
					var iMsgBoxLen = 0;
					while (GetChar(szMsgBox, iMsgBoxLen)) ++iMsgBoxLen;
...
global func ShoutMsg(sMessage, pSpeaker, szPortrait, iExcludePlayer, fPermanent)
{
  ...
  if(iExcludePlayer)
  {
    ...
    while(i--)
      if(GetPlayerByIndex(i,C4PT_User)!=iExcludePlayer-1) CustomMessage(Format("%s<c %x>%s:</c> %s", szPermanent, dwClr, GetName(pSpeaker), sMessage), 0, GetPlayerByIndex(i,C4PT_User), 90,70, 0xffffff, MD69, szPortrait, 0);
  }
  else CustomMessage(Format("%s<c %x>%s:</c> %s", szPermanent, dwClr, GetName(pSpeaker), sMessage), 0, -1, 90,70, 0xffffff, MD69, szPortrait, 0);
