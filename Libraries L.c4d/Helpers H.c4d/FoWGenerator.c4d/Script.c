...
protected func UpdateTransferZone()
  ...
  if (GetAction() == "Active") SetPlrViewRange(Min((w+h)/-2+40, -1));
  }
...
public func Activate()
  ...
  while (pObj = FindObject(0, w/-2,h/-2,w,h, 0, 0,0, 0, pObjExclude))
    if (GetID(pObj) != _WSC && GetID() != CHST && GetID(Contained()) != CHST) // Schilder, Kisten und Gegenstände darin überspringen
...
protected func CheckClonk()
  {
  var o; while (o = FindObject(0, w/-2-FOW_PlayerSearchRangeX, h/-2-FOW_PlayerSearchRangeY, w+FOW_PlayerSearchRangeX*2, h+FOW_PlayerSearchRangeX*2, OCF_CrewMember, 0,0, NoContainer(), o))
