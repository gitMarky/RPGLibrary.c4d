...
/*
		...
		if(!(qIndex%iEntriesPerPage) && qIndex)
		{
			...
			//{
			//	iIndex++;
			//	PushBack([iIndex,iMenuPageParent, Format("Zurück zu Seite %d",iMenuPage),"Zurück", 0, [LMM2,0,0,2,0,iMenuPageParent+1], [false,0,0,0,false,-1,"Journal"] ],aDialogue);
			//}
			iMenuPage++;
			iIndex++;
		}
...
*/
		...
		if(!(qIndex%iEntriesPerPage) && qIndex)
		{
			...
			{
				iIndex++;
				PushBack([iIndex,iMenuPageParent/*Old*/, Format("Zurück zu Seite %d",iMenuPage-1),"Zurück", 0, [LMM2,0,0,2,0,iMenuPageParentOld+1], [false,0,0,0,false,-1,"Journal"] ],aDialogue);
			}
			//iMenuPage++;
			//iIndex++;
		}
