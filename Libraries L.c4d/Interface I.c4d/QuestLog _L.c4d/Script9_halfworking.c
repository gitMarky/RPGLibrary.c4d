...
/*
	// Zwischenspeicher
	var qIndex = GetArrayItemPosition(szName,aQuestIndex);
	var desc = aQuestIndex[qIndex];
	var read = 0;

	// altes Zeug löschen
	DeleteArrayItem(qIndex, aQuestIndex);
	DeleteArrayItem(qIndex, aQuestDesc);
	DeleteArrayItem(qIndex, aQuestRead);

	// alles an den Anfang stellen
	PushFront(szName,aQuestIndex);
	PushFront(desc,aQuestDesc);
	PushFront(0,aQuestRead);*/
}
...
public func ConstructDialogue( pPlayer )
{
	...
	PushBack([0,-1,0,ColorizeString(Format("Offene Quests (%d/%d)",iMenuPage,iMenuPages),QuestColorMenu()),0,SCRG,[false,QuestColorMenu(),0,0,false,-1,"Journal"]],aDialogue);
	...
	PushBack([iIndex,aPagesIndex,"Abgeschlossene Quests",ColorizeString(Format("Abgeschlossene Quests (%d/%d)",iMenuPage,iMenuPages),QuestColorMenu()),0,[QuestIconDone(),QuestColorMenu()],[false,QuestColorMenu(),0,0,false,-1,"Journal"]],aDialogue);
	iIndex++;
	...
	PushBack([iIndex,aPagesIndex,"Gescheiterte Quests",ColorizeString(Format("Gescheiterte Quests (%d/%d)",iMenuPage,iMenuPages),QuestColorMenu()),0,[QuestIconFail(),QuestColorMenu()],[false,QuestColorMenu(),0,0,false,-1,"Journal"]],aDialogue);
	iIndex++;
		...
		for( var qIndex = 0; qIndex < GetLength(aQuestIndex); qIndex++)
		{
			...
			if(!(qIndex%iEntriesPerPage) && (qIndex||iEntriesPerPage==1))
			{
				if( iMenuPage != iMenuPages)
				{
					...
					PushBack([iIndex,iMenuPageParent,"Weiter",ColorizeString(Format("%s Quests (%d/%d)",qstring,iMenuPage+1,iMenuPages),QuestColorMenu()),0,[LMM2,QuestColorMenu(),0,2,4],[false,QuestColorMenu(),0,0,false,-1,"Journal"]],aDialogue);
