...
public func UpdatePicture(idSymbol, object pObj)
{
	var offset = 500 * Max
	(
		GetDefCoreVal("Picture", "DefCore", idSymbol, 2),
		GetDefCoreVal("Picture", "DefCore", idSymbol, 3)
	);
	// setzt die Picture-Grafik richtig
	var gfx = "";
	if (pObj)
		gfx = pObj->GetGraphics();
	...
	if (pObj)
		if (pObj != this)
			if (GetClrModulation(pObj))
				SetClrModulation(GetClrModulation(pObj), this, 2);
}
