...
protected func Initialize()
{
	...
	AddEffect("MonitorQuest",this,50,20,this);
}
...
protected func ProcessEvents( aEvents, object pActivePlayer )
{
	// keine Bedingungen sind immer erfüllt
	...
	for(var i = 0; i < len; i++)
	{
			if( GetType( aEvents ) == C4V_String )
				CheckCondition( aEvents, pActivePlayer, aClonks[i] );
			else
			{
				for( aEv in aEvents )
					CheckCondition( aEv, pActivePlayer, aClonks[i] );
			}
	}
}
