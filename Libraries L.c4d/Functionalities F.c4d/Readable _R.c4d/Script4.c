...
public func printChildren(int parentid, int depth, array children)
{
	...
	var option, answer;
	//Log("ao: %d: %v", node[2], aDialogue[index]);
	...
	{
			var child = node[1][j];
		//Log("loop detected %d? %d %d %v", depth,child,index,children);
		var cancel = false;
		//Log("loop? %v %v",parentid, children);
		//if(GetArrayItemPosition(parentid,children)>= 0 )
		Log("loop? %v %v",parentid, children);
		if(GetArrayItemPosition(parentid,children)>= 0 )
		//if(GetArrayItemPosition(index,tree[child][1])>= 0 )
		{
			//Log("loop detected %d? %d %d %v", depth,parent,index,node[1]);
//			Log("loop detected %d? %d %d %v", depth,parent,index,tree[child][1]);
		 if(depth>1) cancel = true;
		}
		if(cancel) continue;
