...
// Setzt den Startdialog eines Objekts
global func SetStartDialogue(object pTarget, int iDlg)
{
	var talker = FindObject2(Find_ID(TALK), Find_Action("Attach"), Find_ActionTarget(pTarget));
	
	if (talker)
		talker->~SetStartDialogueEx(iDlg);
	else
...
global func CheckObjectOrder(bool fCorrect)
{
	...
	while (obj = FindObject(0, 0, 0, 0, 0, 0, 0, 0, 0, obj))
	{
		...
		else if (c2 > cat)
		{
			Log
			(
				"!!! Object %d (%s) is in wrong sort-order %d!",
				ObjectNumber(obj),
				obj->GetName(),
				c2
			);
...
global func GetLastMsgWaitTime()
{
	return 0;
}
...
global func & GetOriginalColorDw(object pObj)
{
	...
	if (!pObj)
		if (!(pObj = this))
			return;
	var iEff;
	if (iEff = GetEffect("IntGlobalClrMod", pObj))
		return EffectVar(3, pObj, iEff);
...
global func FxIntGlobalClrModStart(object pTarget, int iEff, int iTemp, int iToVal, int iStep)
{
	...
	else
	{
		...
		if (!EffectVar(3, pTarget, iEff))
			EffectVar(3, pTarget, iEff) = 0xFFFFFF;
	}
...
{
	if (szNewEffect == "IntGlobalClrMod")
		return -2;
}
...
global func HideObject(object pObj)
{
	if (!pObj)
		if (!(pObj = this))
			return;
	if (!g_GlobalContainer)
		g_GlobalContainer = CreateObject(AT69, 50, 50, -1);
	Enter(g_GlobalContainer, pObj);
}
  
global func ShowObject(object pObj)
{
	if (Contained(pObj) == g_GlobalContainer)
		Exit(pObj);
}
...
global func ShrU(x, n)
{
	return (x & 2147483647) >> n | (x < 0) << 31 - n;
}
global func ShlU(x, n)
{
	return (x & 2147483647) << n;
}
